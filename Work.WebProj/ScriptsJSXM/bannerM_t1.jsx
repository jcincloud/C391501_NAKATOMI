	...
	render:function(){
		return (
				...
				<tr>
					...
					<td className="text-center">{this.filter(this.props.itemData.category,'bannerCategory')}</td>
					<td className="text-center">{this.props.itemData.sort}</td>
					<td className="text-center">{this.props.itemData.i_Hide ? <span className="label label-default">隱藏</span>:<span className="label label-primary">顯示</span>}</td>
					<td className="text-center">{this.filter(this.props.itemData.i_Lang,'LangData')}</td>
				</tr>
			);
		}
});
...
var GirdForm = React.createClass({
	...
	insertType:function(){
		this.setState({edit_type:1,fieldData:{i_Lang:'zh-TW',type:1,category:0}});
	},
	...
	render: function() {
		...
		if(this.state.edit_type==0)
		{
			...
			<div>
		        ...
		        <div className="alert alert-warning">
		           	<p>分類為照片的資料，請最少上傳 <strong className="text-danger">3</strong>張。</p>
                </div>
							...
							<div className="table-filter">
								<div className="form-inline">
									<div className="form-group">
										...
										<option value="">All</option>
											{
												commData.bannerCategory.map(function(itemData,i) {
													return <option key={itemData.val} value={itemData.val}>{itemData.label}</option>;
												})
											}
										</select>
						...
						<table>
							<thead>
								<tr>
									...
									<th className="col-xs-3 text-center">標題</th>
									<th className="col-xs-1 text-center">分類</th>
									<th className="col-xs-1 text-center">排序</th>
									<th className="col-xs-1 text-center">狀態</th>
									<th className="col-xs-2 text-center">語系</th>
								</tr>
							</thead>
							<tbody>
								{
		...
		else if(this.state.edit_type==1 || this.state.edit_type==2)
		{
			...
			var video_url_html=null;
			if(fieldData.category==1){
				video_url_html=(
						<div className="form-group">
							<label className="col-xs-1 control-label">Youtube影片網址</label>
							...
							<small className="help-inline col-xs-5">請輸入完整youtube嵌入原始碼</small>
						</div>
					);
			}
			...
			outHtml=(<div>
					...
					<div className="alert alert-warning">
		           		<p>1.分類為照片-未上傳圖片時,將 <strong className="text-danger">不會顯示在前台</strong>。</p>
		           		<p>2.分類為影片-未上傳圖片及未輸入影片連結時,將 <strong className="text-danger">不會顯示在前台</strong>。</p>
                	</div>
					...
					<div className="form-group">
						...
						<div className="col-xs-6">
							...
							url_upload={gb_approot + 'Sys_Active/Banner/axFUpload'}
							url_list={gb_approot+'Sys_Active/Banner/axFList'}
							url_delete={gb_approot+'Sys_Active/Banner/axFDelete'}
							url_sort={gb_approot+'Sys_Active/Banner/axFSort'}
							/>
						</div>
						<small className="help-inline col-xs-5 text-danger">限 1 張圖片，檔案大小限制 5MB(5000KB)</small>
					</div>
						...
						<div className="form-group">
							...
							<small className="help-inline col-xs-5">最多10字</small>
						</div>

												<div className="form-group">
							<label className="col-xs-1 control-label">語系</label>
