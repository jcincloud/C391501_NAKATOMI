...
var GirdForm = React.createClass({
	...
	handleSubmit: function(detailData,e) {
		...
		this.state.fieldData.活動花絮內容 = detailData;
		if(this.state.edit_type==1){
			jqPost(gb_approot + 'api/Active',this.state.fieldData)
			...
			jqPut(gb_approot + 'api/Active',this.state.fieldData)
			.done(function(data, textStatus, jqXHRdata) {
				if(data.result){
	...
	deleteSubmit:function(e){
		...
		jqDelete(gb_approot+'api/Active?' + ids.join('&'),{})			
		.done(function(data, textStatus, jqXHRdata) {
			if(data.result){
	...
	gridData:function(page){
		...
		return jqGet(gb_approot + 'api/Active',parms);
	},
	...
	updateType:function(id){
		jqGet(gb_approot + 'api/Active',{id:id})
		.done(function(data, textStatus, jqXHRdata) {
		...
		else if(this.state.edit_type==1 || this.state.edit_type==2)
		{
			...
			<div>
				...
				<form className="form-horizontal" onSubmit={this.handleSubmit}>
					<div className="form-group">
						<label className="col-xs-1 control-label">主標題</label>
						...
						<div className="col-xs-3">
							<span className="has-feedback">
								<InputDate id="active_date" name="活動日期" onChange={this.changeFDValue} value={this.state.fieldData.活動日期} />
...
var SubGirdForm = React.createClass({
	...
	deleteSubmit:function(e){
		...
		jqDelete(gb_approot+'api/ActiveContent?' + ids.join('&'),{})			
		.done(function(data, textStatus, jqXHRdata) {
			if(data.result){
				tosMessage(null,'刪除完成',1);
				this.queryGridData(0);
	...
	gridData:function(){
		...
		return jqGet(gb_approot + 'api/ActiveContent',parms);
	},
	...
	deleteItem:function(i){
		...
		if(data.edit_state==0){
			...
			jqDelete(gb_approot+'api/ActiveContent?ids=' + data.流水號,{})
			.done(function(data, textStatus, jqXHRdata) {
				if(data.result){
					newState.gridData.splice(i,1);
					this.setState(newState);
		...
		<div className="panel">
			...
			<div id={'item-' + this.props.ikey} className="panel-collapse collapse in">
				<div className="panel-body">
					...
					<div className="form-group">
						<label className="col-xs-1 control-label">次標題</label>
						<div className="col-xs-3">
							<input type="text" 
							className="form-control"	
							value={fieldData.標題}
						...
						<div className="col-xs-3">
							<span className="has-feedback">
								<InputDate
								id={'active_date_' + this.props.ikey}
								name="活動日期"
								onChange={this.changeFDValue} 
								value={this.state.fieldData.活動日期} />
...
var MasterFileUpload = React.createClass({
	...
	deleteFile:function(filename){
		jqPost(gb_approot+'Sys_Active/Active/aj_FDelete',{
			id:this.props.MainId,
			fileKind:this.props.FileKind,
			filename:filename
		})			
	...
	getFileList:function(){
		jqPost(gb_approot+'Sys_Active/Active/aj_FList',{
			id:this.props.MainId,
			fileKind:this.props.FileKind
		})			
		.done(function(data, textStatus, jqXHRdata) {
	...
	createFileUpLoadObject:function(){
			...
			var r_this = this;
		  	var uploader = new ss.SimpleUpload({
		        button: btn,
		        url: gb_approot + 'Sys_Active/Active/aj_FUpload',
		        data:{
		        	id:this.props.MainId,
		        	fileKind:this.props.FileKind
		        },
